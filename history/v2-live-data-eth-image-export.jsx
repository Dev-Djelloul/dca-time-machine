...
// Downsample daily [timestamp, price] pairs from CoinGecko into "YYYY-MM" monthly averages
...
export default function DCATimeMachine() {
  ...
  const [monthlySeries, setMonthlySeries] = useState(ASSETS.BTC.fallback);

  // Attempt live fetch from CoinGecko; gracefully fall back on any failure.
  useEffect(() => {
    let cancelled = false;
    ...
    fetch(url)
      ...
      .then((json) => {
        ...
        const monthly = toMonthlySeries(json.prices);
        if (monthly.length < 12) throw new Error("too sparse");
        setMonthlySeries(monthly);
        setDataSource("live");
      })
      .catch(() => {
        if (cancelled) return;
        setMonthlySeries(cfg.fallback);
  ...
  // Keep startMonth valid whenever the series changes (asset switch / live load)
  useEffect(() => {
  ...
  const startIdx = useMemo(
    () => monthlySeries.findIndex(([k]) => k === startMonth),
    [monthlySeries, startMonth]
  );
  ...
  const series = useMemo(() => {
    ...
    const rows = slice.map(([key, price]) => {
  ...
  }, [startIdx, monthly, monthlySeries]);
  ...
  }, [done, finalRow, dcaFinal, totalInvested, lumpFinal, multiple, monthly, startMonth, lastMonth, assetCfg, series.rows.length]);
  ...
  const exportImage = useCallback(() => {
    ...
    ctx.fillText("✂ — REÇU DE TÉLÉPORTATION FINANCIÈRE — ✂", width / 2, 34);
    ...
    a.href = dataUrl;
    a.download = `dca-time-machine-${asset.toLowerCase()}-${startMonth}.png`;
    a.click();
  }, [roastLines, asset, startMonth]);
    ...
    >
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginBottom: 4 }}>
          ...
          <span style={{ color: "#5A6B7A", fontSize: 12 }}>v2.0</span>
          <span
            style={{
              marginLeft: "auto",
              fontSize: 10.5,
        ...
        >
          ...
          <button
            ...
            disabled={running || dataSource === "loading"}
            style={{
              marginTop: 18,
              width: "100%",
              padding: "12px 16px",
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={visibleData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                ...
                <XAxis
                  ...
                  interval={Math.max(0, Math.floor(series.rows.length / 6))}
        ...
        {/* Receipt */}
        {done && (
          <div style={{ position: "relative" }} ref={receiptRef}>
            <div
              style={{
                background: "#F2ECD9",
                color: "#20261F",
